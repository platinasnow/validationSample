...
public class UserValidation  implements Validator{
	...
	public void validate(Object target, Errors errors) {
		...
		if(user.getUserId().length() < 6 || user.getUserId().length() > 10){
			errors.rejectValue("userId", "userId.size", "아이디는 6~10 글자 사이로 만들어주세요.");
		}
		if(user.getPassword().length() < 8 || user.getPassword().length() > 16){ 
			errors.rejectValue("password", "password.size", "비밀번호는 8~16 글자 사이로 만들어주세요.");
		}
		if(!user.getPassword().equals(user.getPwConfirm()) ){ 
			errors.rejectValue("pwConfirm", "pwConfirm.equal", "비밀번호가 일치하지 않습니다.");
		}
		if(!StringUtil.isValidEmail(user.getEmail())){
			errors.rejectValue("email", "email.match", "이메일 형식이 올바르지 않습니다.");
		}
	}
}
